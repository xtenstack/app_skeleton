...
# The bind-mounted host directories (logs/, public/files/) can arrive
# owned by whoever created them on the host — root, if a deploy script
# does `mkdir -p data` over SSH as root, which is exactly what happened
# on the first prod deploy (2026-07-30) and silently broke error logging:
# www-data (php-fpm's run user, uid 33) had no write access, so
...
chown -R www-data:www-data /app/logs /app/public/files
