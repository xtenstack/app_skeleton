-- MySQL/MariaDB port of postgresql/020_kb_enquiry_types.sql; keep the two in step.
...
-- No partial indexes on MySQL/MariaDB: a stored generated column holding
-- the name only while the row is live (see 019 for the same pattern).
ALTER TABLE kb_enquiry_types
    ADD COLUMN name_active VARCHAR(50) AS (CASE WHEN deleted_at IS NULL THEN name END) STORED;
CREATE UNIQUE INDEX kb_enquiry_types_name_active_idx ON kb_enquiry_types (name_active);
