-- MySQL/MariaDB port of postgresql/017_ticket_intake_token.sql; keep the two in step.
...
ALTER TABLE tickets ADD COLUMN intake_submitted_at DATETIME;

-- MySQL/MariaDB have no partial indexes, but a UNIQUE index already allows
-- any number of NULLs, which is all the Postgres WHERE clause was for.
CREATE UNIQUE INDEX tickets_intake_token_idx ON tickets (intake_token);
