-- SQLite port of postgresql/022_ticket_kb_enquiry_type.sql; keep the two in step.
...
-- SQLite has no ADD CONSTRAINT; the FK goes on the ADD COLUMN instead.
ALTER TABLE tickets ADD COLUMN kb_enquiry_type_id INTEGER REFERENCES kb_enquiry_types(id);
